...
async def main():
    ...
    transport = moteus_pi3hat.Pi3HatRouter(
        servo_bus_map={can_bus: [motor_ids]},
    )
    ...
    controllers = {
        id: moteus.Controller(id=id, transport=transport, query_resolution=qr) for id in motor_ids
    }
    for id in motor_ids:
        print(id)
        s = moteus.Stream(controllers[id], verbose=True)
        max_velocity = await s.command(
            b'conf set servo.max_velocity ' + str(MAXVEL).encode('utf-8')
        )
        max_power = await s.command(b'conf set servo.max_power_W ' + str(MAXPOW).encode('utf-8'))
        max_current = await s.command(
            b'conf set servo.max_current_A ' + str(MAXCUR).encode('utf-8')
        )
        ...
        flux_brake_voltage = await s.command(
            b'conf set servo.flux_brake_min_voltage ' + str(FBV).encode('utf-8')
        )
        pos_inf_lim = await s.command(
            b'conf set servopos.position_min ' + str(inf_pos_limit).encode('utf-8')
        )
        pos_sup_lim = await s.command(
            b'conf set servopos.position_max ' + str(sup_pos_limit).encode('utf-8')
        )
    ...
    while True:
        ...
        command = []
        for id in motor_ids:
            command.append(controllers[id].make_position(position=pos, velocity=vel, query=True))
    ...
    if (
        len(msr_positions) == len(cmd_positions)
        and len(cmd_positions) == len(times)
        and len(times) == len(msr_positions)
    ):
