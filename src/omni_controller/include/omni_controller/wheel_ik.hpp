...
public:
    ...
    virtual void configure(const WheelIKConfig & config,
                           const std::vector<std::string> & wheel_names) = 0;
...
{
public:
    void configure(const WheelIKConfig & config,
                   const std::vector<std::string> & wheel_names) override;
