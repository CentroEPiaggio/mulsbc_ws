...
// using Get_Function = std::function<moteus::QueryResultV2 ( std::vector<Reply>& replies, int bus,
// int id, int opt,int& err,int provided_msg)>; using Policy_Function = std::function<void( bool
// msg_valid, bool msg_coplete, Command* cmd_d)>;
...
public:
    ...
    int try_consume(std::chrono::_V2::system_clock::time_point* now)
    {
        ...
        *now = std::chrono::high_resolution_clock::now();
        // RCLCPP_INFO(rclcpp::get_logger("CALLBACK"),"try consume %d",
...
private:
    // std::vector<std::unique_ptr<pi3hat_hw_interface::actuator_manager::Actuator_Manager>>
    // actuators_;
    ...
    std::map<int, std::unique_ptr<pi3hat_hw_interface::actuator_manager::Actuator_Manager>>
        actuators_;
    std::map<int, std::unique_ptr<pi3hat_hw_interface::power_dist_manager::Distributor_Manager>>
        distributors_;
