...
public:
    Actuator_Manager(CanFdFrame* command_frame) { cmd_frame_ = command_frame; };
    ~Actuator_Manager();
    void
    SetActuatorParam(const ActuatorOptions opt, uint16_t id, uint16_t bus, std::string jnt_name)
    {
        id_ = id;
        bus_ = bus;
        jnt_name_ = jnt_name;
    ...
    void setQueryFormat(const ActuatorQuery query_format)
    {
        ...
        if (query_format.position_error != 0) {
            ...
            query_format_.extra[extra_count].register_number =
                mjbots::moteus::kControlPositionError;
            extra_count++;
        }
        if (query_format.velocity_error != 0) {
            query_format_.extra[extra_count].resolution = parse_res(query_format.velocity_error);
            query_format_.extra[extra_count].register_number =
                mjbots::moteus::kControlVelocityError;
            extra_count++;
        }
        ...
        if (query_format.second_encoder_position != 0 && se_source_ != -1) {
            query_format_.extra[extra_count].resolution =
                parse_res(query_format.second_encoder_position);
            if (se_source_ == 0)
                query_format_.extra[extra_count].register_number =
                    mjbots::moteus::kEncoder0Position;
            else if (se_source_ == 1)
                query_format_.extra[extra_count].register_number =
                    mjbots::moteus::kEncoder1Position;
            else if (se_source_ == 2)
                query_format_.extra[extra_count].register_number =
                    mjbots::moteus::kEncoder2Position;
            else
        ...
        if (query_format.second_encoder_velocity != 0 && se_source_ != -1) {
            query_format_.extra[extra_count].resolution =
                parse_res(query_format.second_encoder_velocity);
            if (se_source_ == 0)
                query_format_.extra[extra_count].register_number =
                    mjbots::moteus::kEncoder0Velocity;
            else if (se_source_ == 1)
                query_format_.extra[extra_count].register_number =
                    mjbots::moteus::kEncoder1Velocity;
            else if (se_source_ == 2)
                query_format_.extra[extra_count].register_number =
                    mjbots::moteus::kEncoder2Velocity;
            else
                throw std::runtime_error("se source is set wrongly");
            extra_count++;
        }
        ...
        // RCLCPP_INFO(rclcpp::get_logger("Actuator_Manager"),"Set Query Format extra 0 %d
        // %d",query_format_.extra[0].register_number,query_format_.extra[0].resolution);
    ...
    void setSecondEncoderSource(unsigned int se_source) { se_source_ = se_source; };
    std::string get_joint_name() { return jnt_name_; };
    uint16_t GetActuatorId() { return id_; };
    ...
    void SendDiagnosticCommandAndVerify(const std::string& cmd)
    {
        ...
        for (int attempt = 1; attempt <= max_retries; attempt++)
        {
            ...
            if (r == "OK")
            {
                RCLCPP_INFO(rclcpp::get_logger("Actuator_Manager"),
                    "Parameter confirmed (attempt %d) motor id %d bus %d",
                    attempt, id_, bus_);
                return;
            }
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("Actuator_Manager"),
                    "No response (attempt %d/%d)",
                    attempt, max_retries);
            }
            else
            {
                RCLCPP_WARN(rclcpp::get_logger("Actuator_Manager"),
                    "Unexpected response (attempt %d/%d): %s",
                    attempt, max_retries, response.c_str());
            }
        ...
        RCLCPP_ERROR(rclcpp::get_logger("Actuator_Manager"),
            "FAILED to confirm command after %d attempts for motor id %d bus %d",
            max_retries, id_, bus_);
    }
    ...
    mjbots::moteus::Resolution parse_res(int res)
    {
        if (res == 0)
            ...
            throw std::runtime_error(
                "Wrong resolution format are available just [0,1,2,3,4] not " + std::to_string(res)
            );
    }
...
public:
    Distributor_Manager(CanFdFrame* command_frame) { cmd_frame_ = command_frame; };
    void SetDistributorParam(uint16_t id, uint16_t bus, std::string dist_name)
    {
        if (bus != 5) {
    ...
    void
    setQueryFormat(const pi3hat_hw_interface::power_dist_manager::DistributorQuery query_format)
    {
