...
template <typename T> class ElementInfo {
public:
    T get_configurable() { return configurable_; }
...
protected:
    ...
    {
        for (std::vector<std::string>::iterator i = parsed_params_.begin();
             i != parsed_params_.end(); i++) {
            if (*i == param_name)
    ...
    {
        for (std::vector<std::string>::iterator i = available_base_params_.begin();
             i != available_base_params_.end(); i++) {
            if (*i == param_name)
                return;
        }
        for (std::vector<std::string>::iterator i = available_extra_params_.begin();
             i != available_extra_params_.end(); i++) {
            if (*i == param_name)
                return;
        }
...
public:
    ...
    QueryFormatInfo()
    {
        ...
        // {
        //     this->configurable_.extra[i].register_number =
        //     mjbots::moteus::detail::numeric_limits<int16_t>::max();
        //     this->configurable_.extra[i].resolution = Resolution::kIgnore;
        // }
        // init available params
        this->available_base_params_ = {"position_res",  "velocity_res",
                                        "torque_res",    "q_current_res",
                                        "d_current_res", "abs_position_res",
                                        "power_res",     "motor_temperature_res",
                                        "voltage_res",   "temperature_res"};
        this->available_extra_params_ = {
            "position_error_res",          "velocity_error_res",          "torque_error_res",
            "second_encoder_position_res", "second_encoder_velocity_res",
    ...
    int parse_res(int res)
    {
        if (res == 0)
            ...
            throw std::runtime_error(
                "Wrong resolution format are available just [0,8,16,32,64] parsed " +
                std::to_string(res)
            );
    }
    void parse_map(std::unordered_map<std::string, std::string> pars) override
    {
        ...
        for (std::unordered_map<std::string, std::string>::iterator i = pars.begin();
             i != pars.end(); i++) {
            ...
            if (i->first.compare(this->available_base_params_[0]) == 0) {
                ...
                // {
                //     configurable_.extra[extra_count_].resolution =
                //     parse_res(std::stoi(i->second)); RCLCPP_INFO(rclcpp::get_logger("Query
                //     Resolution Parser DEBUG"),"Parsing extra param %s with res %s,
                //     %d",i->first.c_str(),i->second.c_str(),parse_res(std::stoi(i->second)));
                //     configurable_.extra[extra_count_].register_number =
                //     mjbots::moteus::Register::kControlPositionError;
                //     RCLCPP_INFO(rclcpp::get_logger("Query Resolution Parser DEBUG"),"Extra %d :
                //     register number %d with res
                //     %d",extra_count_,configurable_.extra[extra_count_].register_number,static_cast<int>(configurable_.extra[extra_count_].resolution));
                //     extra_count_ ++;
                // }
                // else
                ...
                if (se_source_ == -1) {
                    RCLCPP_WARN(
                        rclcpp::get_logger("Query Resolution Parser"),
                        "No second encoder will be used"
                    );
                } else
                ...
                // {
                //     configurable_.extra[extra_count_].resolution =
                //     parse_res(std::stoi(i->second)); if(se_source_ == 0)
                //         configurable_.extra[extra_count_].register_number =
                ...
                //     else
                //         throw std::runtime_error("second_encoder_source must be 0, 1 or 2");
                //     // RCLCPP_INFO(rclcpp::get_logger("Query Resolution Parser"),"Extra %d :
                //     register number %d with res
                //     %d",extra_count_,configurable_.extra[extra_count_].register_number,static_cast<int>(configurable_.extra[extra_count_].resolution));
                //     extra_count_ ++;
            ...
            // else
            // {
            //     RCLCPP_WARN(rclcpp::get_logger("Query Resolution Parser"),"The param %s do not
            //     exist", i->first.c_str()); throw std::runtime_error("Unable to parse
            //     parameters");
            // }
        }
        ...
        for (size_t i = 0; i < changes_.size(); i++) {
            if (!changes_[i])
                RCLCPP_WARN(
                    rclcpp::get_logger("Query Resolution Parser"),
                    "The base query conf %s is set to default", available_base_params_[i].c_str()
                );
        }
    }
    int get_secodn_encoder_source() { return se_source_; };
...
public:
    Pi3hatConfInfo()
    {
        this->available_base_params_ = {"timeout_ns",       "min_tx_wait_ns", "rx_baseline_wait_ns",
                                        "rx_extra_wait_ns", "CPU_affinity",   "request_attitude"};
        this->available_extra_params_ = {
            "mounting_deg_roll", "mounting_deg_pitch", "mounting_deg_yaw", "attitude_hz"
        };
        changes_.fill(false);
    };
        ...
        for (auto jnt : info.joints) {
            ...
            try {
                auto res = configurable_.servo_map.insert(
                    {std::stoi(jnt.parameters.at("id")), std::stoi(jnt.parameters.at("bus"))}
                );
                if (!res.second)
                    throw std::runtime_error("Error, motors id are not unique");
            } catch (const std::exception& e) {
                RCLCPP_ERROR(
                    rclcpp::get_logger("Query Resolution Parser"), "Parsing jnt %s ID throw: %s",
                    jnt.name.c_str(), e.what()
                );
    ...
    void parse_map(std::unordered_map<std::string, std::string> pars) override
    {
        for (std::unordered_map<std::string, std::string>::iterator i = pars.begin();
            ...
            if (i->first.compare(available_base_params_[0]) == 0) {
                ...
                RCLCPP_WARN(
                    rclcpp::get_logger("Pi3Hat Transport Parser"), "The param %s do not exist",
                    i->first.c_str()
                );
        ...
        for (size_t i = 0; i < changes_.size(); i++) {
            if (!changes_[i])
                RCLCPP_WARN(
                    rclcpp::get_logger("Pi3Hat Transport Parser"),
                    "The pi3hat  conf %s is set to default", this->available_base_params_[i].c_str()
                );
        }
    }
...
public:
    ...
    void parse_map(std::unordered_map<std::string, std::string> pars) override
    {
        ...
        for (size_t i = 0; i < changes_.size(); i++) {
            if (!changes_[i])
                RCLCPP_WARN(
                    rclcpp::get_logger("Moteus Driver Param"),
                    "The moteus conf %s is set to default", available_base_params_[i].c_str()
                );
        }
    };
...
public:
    PDQueryFormatInfo()
    {
        ...
        available_extra_params_ = {"state", "switch_state", "lock_time", "boot_time"};
        changes_.fill(false);
    }
    void parse_map(std::unordered_map<std::string, std::string> pars) override
    {
