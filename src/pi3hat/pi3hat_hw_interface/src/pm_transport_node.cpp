...
public:
        ...
        for (int i = 0; i < count; i++) {
            {
                ...
                t_->Cycle(
                    frames.data(), frames.size(), &replies, &att_, nullptr, nullptr,
                    b_clb.callback()
                );
            ...
            for (auto rep : replies) {
                if (rep.source == 32) {
                    ...
                    RCLCPP_INFO(
                        this->get_logger(), "pos %f::::vel %f, %d", res.output_voltage,
                        res.output_current, res.state
                    );
                }
            }
            // sleep_for(5ms);
    ...
    void start_control(float vel)
    {
        ...
        RCLCPP_INFO(
            this->get_logger(), "the source is %d, id is %d, the bus is %d", cmd_frame.source,
            cmd_frame.arbitration_id, cmd_frame.bus
        );
        RCLCPP_INFO(this->get_logger(), "query has been made");
        cmd_frames_.push_back(cmd_frame);
        timer_ = this->create_wall_timer(100ms, std::bind(&MoteusPi3hatNode::timer_cbk, this));
    }
    void test_encoder_validity()
    {
        ...
        if (result.has_value()) {
            ...
            RCLCPP_INFO(
                this->get_logger(), "validity %s",
                ((test & (1 << 0)) != 0) && ((test & (1 << 1)) != 0) ? "true" : "false"
            );
            RCLCPP_INFO(
                this->get_logger(), "validity %s",
                ((test & (1 << 2) != 0)) && ((test & (1 << 3)) != 0) != 0 ? "true" : "false"
            );
            RCLCPP_INFO(
                this->get_logger(), "validity %s",
                ((test & (1 << 4) != 0)) && ((test & (1 << 5)) != 0) != 0 ? "true" : "false"
            );
            ...
            if (res == -1) {
                ...
                for (auto& rep : replies_) {
                    if (rep.source == 32) {
                        ...
                        RCLCPP_INFO(
                            this->get_logger(), "volt %f::::cur %f, %d", res.output_voltage,
                            res.output_current
                        );
                    }
                }
                t_->Cycle(
                    cmd_frames_.data(), cmd_frames_.size(), &replies_, &att_, nullptr, nullptr,
                    clb_as_.callback()
                );
