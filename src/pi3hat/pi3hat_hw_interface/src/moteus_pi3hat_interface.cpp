...
void MoteusPi3Hat_Interface::sendStopCycles(const char* caller)
{
    ...
    if (!motors_stopped_.compare_exchange_strong(expected, true)) {
        RCLCPP_INFO(
            rclcpp::get_logger(LOGGER_NAME), "[%s] Motors already stopped, skipping", caller
        );
        return;
    }
    ...
    if (!pi3hat_transport_) {
        RCLCPP_WARN(
            rclcpp::get_logger(LOGGER_NAME), "[%s] Transport is null, cannot send stop", caller
        );
        return;
    }
    ...
    {
        auto deadline =
            std::chrono::steady_clock::now() + std::chrono::milliseconds(kStopTimeoutMs);
    ...
    RCLCPP_INFO(rclcpp::get_logger(LOGGER_NAME), "[%s] Sending %d stop cycles", caller, NUM_STOP);
    ...
    for (int i = 0; i < NUM_STOP; i++) {
        ...
        try {
            ...
            RCLCPP_WARN(
                rclcpp::get_logger(LOGGER_NAME), "[%s] Stop cycle %d: %s", caller, i, e.what()
            );
        }
    }

    RCLCPP_INFO(rclcpp::get_logger(LOGGER_NAME), "[%s] Stop cycles complete", caller);
};
MoteusPi3Hat_Interface::~MoteusPi3Hat_Interface() { sendStopCycles("destructor"); };
...
CallbackReturn MoteusPi3Hat_Interface::on_init(const hardware_interface::HardwareInfo& info)
{
    ...
    RCLCPP_INFO(
        rclcpp::get_logger(LOGGER_NAME),
        "Safety thresholds: temp_warn=%.1f C, temp_crit=%.1f C, batt_min=%.1f V, "
        "shutdown_delay=%.1f s",
    ...
    try {
        ...
        RCLCPP_ERROR(
            rclcpp::get_logger(LOGGER_NAME), "Pi3hat Transport parsing extra throw: %s", e.what()
        );
    ...
    for (int i = 0; i < info.joints.size(); i++) {
        ...
        try {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "joint %s has not set the parma type",
        ...
        if (jnt_type.compare("motor") == 0) {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "joint %s type is not valid",
                info.joints[i].name.c_str()
            );
            return CallbackReturn::FAILURE;
        }
    ...
    for (auto i : actuator_index_) {
        ...
        actuators_.emplace(
            std::make_pair(
                i, std::make_unique<pi3hat_hw_interface::actuator_manager::Actuator_Manager>(
                       &command_frames_[i]
                   )
            )
        );
        ...
        try {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "Parsing jnt %s BUS&ID throw: %s",
                jnt_name.c_str(), e.what()
            );
            return CallbackReturn::FAILURE;
        }
        ...
        try {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "Actuator Configuration Parsing throw: %s",
                e.what()
            );
            return CallbackReturn::FAILURE;
        }
        ...
        actuators_[i]->SetActuatorParam(act_opt, id, bus, jnt_name);
        // parse query format
        try {
            query_parser->parse_map(info.joints[i].parameters);
        } catch (const std::exception& e) {
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "Query Format Parsing throw: %s", e.what()
            );
            return CallbackReturn::FAILURE;
        }
    ...
    for (auto i : distributor_index_) {
        // allocate actuator manager
        distributors_.emplace(
            std::make_pair(
                i, std::make_unique<pi3hat_hw_interface::power_dist_manager::Distributor_Manager>(
                       &command_frames_[i]
                   )
            )
        );
        ...
        distributors_[i]->SetDistributorParam(id, bus, jnt_name);
        // parse query format
        try {
            distributor_parser->parse_map(info.joints[i].parameters);
        } catch (const std::exception& e) {
...
CallbackReturn MoteusPi3Hat_Interface::on_configure(const rclcpp_lifecycle::State&)
{
    ...
    for (auto i : actuator_index_) {
        try {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "Actuator %s Configuration throw: %s",
                actuators_[i]->get_joint_name().c_str(), e.what()
            );
            return CallbackReturn::FAILURE;
        }
    }
    for (auto i : distributor_index_) {
        try {
            ...
            RCLCPP_ERROR(
                rclcpp::get_logger(LOGGER_NAME), "Actuator %s Configuration throw: %s",
                distributors_[i]->get_joint_name().c_str(), e.what()
            );
            return CallbackReturn::FAILURE;
        }
    }
    ...
    if (attitude_requested_) {
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_LIN_ACC_X, &imu_linear_acceleration_.x
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_LIN_ACC_Y, &imu_linear_acceleration_.y
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_LIN_ACC_Z, &imu_linear_acceleration_.z
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_ANG_SPD_X, &imu_angular_velocity_.x
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_ANG_SPD_Y, &imu_angular_velocity_.y
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_ANG_SPD_Z, &imu_angular_velocity_.z
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_QUATERN_W, &imu_orientation_.w
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_QUATERN_X, &imu_orientation_.x
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_QUATERN_Y, &imu_orientation_.y
            )
        );
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                "Pi3Hat", hardware_interface::HW_IF_QUATERN_Z, &imu_orientation_.z
            )
        );
    }
    stt_int.emplace_back(
        hardware_interface::StateInterface(
            "Pi3Hat", hardware_interface::HW_IF_VALIDITY_LOSS, &invalid_cycle_
        )
    );
    stt_int.emplace_back(
        hardware_interface::StateInterface(
            "Pi3Hat", hardware_interface::HW_IF_CYCLE_DUR, &cycle_duration_
        )
    );
    stt_int.emplace_back(
        hardware_interface::StateInterface("Pi3Hat", "safety_state", &safety_state_value_)
    );
    for (auto i : actuator_index_) {
        actuators_[i]->ExportSttInt(stt_int);
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                actuators_[i]->get_joint_name(), "package_loss", &packet_loss_[i]
            )
        );
    }
    for (auto i : distributor_index_) {
        ...
        stt_int.emplace_back(
            hardware_interface::StateInterface(
                distributors_[i]->get_joint_name(), "package_loss", &packet_loss_[i]
            )
        );
    }
    return stt_int;
...
MoteusPi3Hat_Interface::export_command_interfaces()
{
...
MoteusPi3Hat_Interface::read(const rclcpp::Time&, const rclcpp::Duration&)
{
    int p;
    if (!first_cycle_) {
    ...
    // Safety monitoring: check temperatures and voltages
    {
        ...
        case SafetyState::CRITICAL: {
            ...
            if (std::chrono::duration<double>(elapsed).count() >= shutdown_delay_) {
                ...
                RCLCPP_ERROR(
                    rclcpp::get_logger(LOGGER_NAME),
                    "SHUTDOWN: Safety shutdown delay (%.1f s) elapsed. Returning ERROR.",
                    shutdown_delay_
                );
            }
            break;
...
hardware_interface::return_type
MoteusPi3Hat_Interface::write(const rclcpp::Time&, const rclcpp::Duration&)
{
    if (invalid_cycle_ == 0.0) {
        bool safety_stop =
            (safety_state_ == SafetyState::CRITICAL || safety_state_ == SafetyState::SHUTDOWN);
        for (auto i : actuator_index_) {
            if (safety_stop)
                actuators_[i]->MakeStop();
            else
...
PLUGINLIB_EXPORT_CLASS(
    pi3hat_hw_interface::moteus_pi3hat_interface::MoteusPi3Hat_Interface,
