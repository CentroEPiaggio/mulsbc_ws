...
CallbackReturn Pi3HatJointGroupController::on_init()
{
    ...
    try {
        ...
        RCLCPP_WARN(
            rclcpp::get_logger(logger_name_),
            "Exception thrown during declaretion of init position with message: %s it gets default "
            "values",
            e.what()
        );
    }
...
CallbackReturn Pi3HatJointGroupController::on_configure(const rclcpp_lifecycle::State&)
{
    ...
    if (!default_init_pos_) {
        ...
        if (init_positions.size() != joint.size()) {
            RCLCPP_ERROR(
                rclcpp::get_logger(logger_name_),
                "'init_pos' and 'joints' can not have different dimension"
            );
            return CallbackReturn::ERROR;
        }
    } else
    ...
    cmd_sub_ = get_node()->create_subscription<CmdMsgs>(
        "~/command", 5, [this](const CmdMsgs::SharedPtr msg) {
...
bool Pi3HatJointGroupController::get_reference()
{
    ...
    if (joints_rcvd_msg_.get()) {
        ...
        for (size_t i = 0; i < joints_rcvd_msg_->name.size(); i++) {
            try {
                // RCLCPP_INFO(get_node()->get_logger(),"pass name
                // %s",joints_rcvd_msg_->name[i].c_str());
                if (joints_rcvd_msg_->position.size() == 0 ||
                    joints_rcvd_msg_->position.size() != joints_rcvd_msg_->name.size())
                    position_cmd_.at(joints_rcvd_msg_->name[i]) = 0.0;
                else {
                    position_cmd_.at(joints_rcvd_msg_->name[i]) = joints_rcvd_msg_->position[i];
                    // RCLCPP_INFO(get_node()->get_logger(),"the %s joint  pos is
                    // %f",joints_rcvd_msg_->name[i].c_str(),joints_rcvd_msg_->position[i]);
                }
                if (joints_rcvd_msg_->velocity.size() == 0 ||
                    joints_rcvd_msg_->velocity.size() != joints_rcvd_msg_->name.size())
                    velocity_cmd_.at(joints_rcvd_msg_->name[i]) = 0.0;
                else
                    velocity_cmd_.at(joints_rcvd_msg_->name[i]) = joints_rcvd_msg_->velocity[i];
                if (joints_rcvd_msg_->effort.size() == 0 ||
                    joints_rcvd_msg_->effort.size() != joints_rcvd_msg_->name.size())
                    effort_cmd_.at(joints_rcvd_msg_->name[i]) = 0.0;
                else
                    effort_cmd_.at(joints_rcvd_msg_->name[i]) = joints_rcvd_msg_->effort[i];
                if (joints_rcvd_msg_->kp_scale.size() == 0 ||
                    joints_rcvd_msg_->kp_scale.size() != joints_rcvd_msg_->name.size())
                    kp_scale_cmd_.at(joints_rcvd_msg_->name[i]) = 0.0;
                else
                    kp_scale_cmd_.at(joints_rcvd_msg_->name[i]) = joints_rcvd_msg_->kp_scale[i];
                if (joints_rcvd_msg_->kd_scale.size() == 0 ||
                    joints_rcvd_msg_->kd_scale.size() != joints_rcvd_msg_->name.size())
                ...
                RCLCPP_ERROR(
                    rclcpp::get_logger(logger_name_),
                    "Raised error during the reference assegnation %s", e.what()
                );
                return false;
            }
        }
...
Pi3HatJointGroupController::update(const rclcpp::Time&, const rclcpp::Duration&)
{
    ...
    for (auto& cmd_int : command_interfaces_) {
        ...
        try {
            if (type == hardware_interface::HW_IF_POSITION)
                ...
                // RCLCPP_ERROR(rclcpp::get_logger(logger_name_),"Interface name not correspond to
                // the declared one");
                return controller_interface::return_type::ERROR;
            }
        } catch (std::exception& e) {
            RCLCPP_ERROR(
                rclcpp::get_logger(logger_name_), "catch error %s during the access to '%s' data",
                e.what(), cmd_int.get_name().c_str()
            );
        }
    }
    ...
    //         {
    //             RCLCPP_ERROR(rclcpp::get_logger(logger_name_),"Interface name not correspond to
    //             the declared one"); return controller_interface::return_type::ERROR;
    //         }
    //     }
    //     catch(std::exception &e)
    //     {
    //         RCLCPP_ERROR(rclcpp::get_logger(logger_name_),"catch error %s during the access to
    //         '%s' data",e.what(),cmd_int.get_name().c_str());
    //     }
    // }
