...
struct EmptyMode {
    struct Command {};
    struct Format {};
    static uint8_t Make(WriteCanData*, const Command&, const Format&) { return 0; }
};
...
struct Query {
    ...
    static uint8_t Make(WriteCanData* frame, const Format& format)
    {
...

        {
            const Resolution kResolutions[] = {
                format.mode,      format.position,  format.velocity,     format.torque,
                format.q_current, format.d_current, format.abs_position, format.power,
            };
            const uint16_t kResolutionsSize = sizeof(kResolutions) / sizeof(*kResolutions);
            WriteCombiner combiner(frame, 0x10, Register::kMode, kResolutions, kResolutionsSize);
        ...
        {
            const Resolution kResolutions[] = {
                format.motor_temperature, format.trajectory_complete, format.home_state,
                format.voltage,           format.temperature,         format.fault,
            };
            const uint16_t kResolutionsSize = sizeof(kResolutions) / sizeof(*kResolutions);
            WriteCombiner combiner(
                frame, 0x10, Register::kMotorTemperature, kResolutions, kResolutionsSize
            );
            for (uint16_t i = 0; i < kResolutionsSize; i++) {
                combiner.MaybeWrite();
            }
...

        {
            ...
            const uint16_t kResolutionsSize = sizeof(kResolutions) / sizeof(*kResolutions);
            WriteCombiner combiner(
                frame, 0x10, Register::kAux1GpioStatus, kResolutions, kResolutionsSize
            );
            for (uint16_t i = 0; i < kResolutionsSize; i++) {
                combiner.MaybeWrite();
            }
...

        {
            const int16_t size = [&]() {
                for (int16_t i = 0; i < kMaxExtra; i++) {
                    if (format.extra[i].register_number == detail::numeric_limits<int16_t>::max()) {
                        return i;
                    }
                }
                return kMaxExtra;
            ...
            for (int16_t this_register = min_register_number, index = 0;
                 this_register <= max_register_number; this_register++) {
            ...
            WriteCombiner combiner(
                frame, 0x10, min_register_number, &resolutions[0], required_registers
            );
        ...
        while (true) {
            ...
            default: {
                if (extra_index < kMaxExtra) {
                    ...
                    result.extra[extra_index].value = ParseGeneric(parser, current.value, res);
                    extra_index++;
                } else {
                    // Ignore this value.
                    parser->ReadInt(res);
    ...
    static double
    ParseGeneric(MultiplexParser* parser, int16_t register_number, Resolution resolution)
    {
        ...
        static constexpr RegisterDefinition PROGMEM kRegisterDefinitions[] = {
            ...
            {
                R::kCommandVelocityLimit,
                1,
                MP::kVelocity,
            },
            {
                R::kCommandAccelLimit,
                1,
                MP::kAcceleration,
            },
...
#endif
        for (uint16_t i = 0; i < sizeof(kRegisterDefinitions) / sizeof(*kRegisterDefinitions);
             i++) {
...
#endif
            if (register_number >= start_reg && register_number < (start_reg + block_size)) {
    ...
    static uint8_t Make(WriteCanData* frame, const Command&, const Format& format)
    {
        const int16_t size = [&]() {
            for (int16_t i = 0; i < kMaxItems; i++) {
                if (format.values[i].register_number == detail::numeric_limits<int16_t>::max()) {
                    return i;
                }
            }
            return kMaxItems;
        ...
        for (int16_t this_register = min_register_number, index = 0;
            ...
            if (format.values[index].register_number == this_register) {
                resolutions[this_register - min_register_number] = format.values[index].resolution;
                index++;
            } else {
                resolutions[this_register - min_register_number] = kIgnore;
            }
...
struct PositionMode {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        WriteCombiner combiner(
            frame, 0x00, Register::kCommandPosition, kResolutions,
        ...
        if (combiner.MaybeWrite()) {
            frame->WriteVoltage(command.fixed_voltage_override, format.fixed_voltage_override);
        }
        if (combiner.MaybeWrite()) {
            frame->WritePwm(command.ilimit_scale, format.ilimit_scale);
        }
        if (combiner.MaybeWrite()) {
            frame->WriteCurrent(command.fixed_current_override, format.fixed_current_override);
        }
        if (combiner.MaybeWrite()) {
            frame->WriteInt(command.ignore_position_bounds, format.ignore_position_bounds);
        }
        return 0;
    }
};
...
struct VFOCMode {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        const Resolution kResolutions[] = {
            format.theta_rad, format.voltage,        kIgnore, kIgnore, kIgnore,
            kIgnore,          format.theta_rad_rate,
        };
        WriteCombiner combiner(
            frame, 0x00, Register::kVFocTheta, kResolutions,
            sizeof(kResolutions) / sizeof(*kResolutions)
        );
...
struct CurrentMode {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        WriteCombiner combiner(
            frame, 0x00, Register::kCommandQCurrent, kResolutions,
            sizeof(kResolutions) / sizeof(*kResolutions)
        );
...
struct StayWithinMode {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        const Resolution kResolutions[] = {
            format.lower_bound,      format.upper_bound,  format.feedforward_torque,
            format.kp_scale,         format.kd_scale,     format.maximum_torque,
            format.watchdog_timeout, format.ilimit_scale, format.ignore_position_bounds,
        };

        WriteCombiner combiner(
            frame, 0x00, Register::kCommandStayWithinLowerBound, kResolutions,
            sizeof(kResolutions) / sizeof(*kResolutions)
        );
...
struct ZeroVelocityMode {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        if (format.kd_scale != kIgnore) {
            ...
            WriteCombiner combiner(
                frame, 0x00, Register::kCommandKdScale, kResolutions,
                sizeof(kResolutions) / sizeof(*kResolutions)
            );
...
struct GpioWrite {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        WriteCombiner combiner(
            frame, 0x00, Register::kAux1GpioCommand, kResolutions,
            sizeof(kResolutions) / sizeof(*kResolutions)
        );
...
struct AuxPwmWrite {
    ...
    static uint8_t Make(WriteCanData* frame, const Command& command, const Format& format)
    {
        ...
        WriteCombiner combiner(
            frame, 0x00, Register::kAux1Pwm1, kResolutions,
            sizeof(kResolutions) / sizeof(*kResolutions)
        );
