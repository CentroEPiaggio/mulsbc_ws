...
public:
    ...
    virtual void Cycle(
        const CanFdFrame* frames, size_t size, std::vector<CanFdFrame>* replies,
    ...
    virtual void
    BlockingCycle(const CanFdFrame* frames, size_t size, std::vector<CanFdFrame>* replies)
    {
...
public:
    ...
    bool operator==(const FileDescriptor& rhs) const { return fd_ == rhs.fd_; }
...
public:
    ThreadedEventLoop() { thread_ = std::thread(std::bind(&ThreadedEventLoop::CHILD_Run, this)); }
...
private:
    void CHILD_Run()
    {
        ...
        while (true) {
            something_cv_.wait(lock, [&]() { return do_something_ || !event_queue_.empty(); });
            do_something_ = false;
...
public:
    ...
    ) override
    {
        ...
        copy->Post(
            std::bind(
                &TimeoutTransport::CHILD_Cycle, this, frames, size, replies, completed_callback
            )
        );
    }
...
protected:
    ...
    virtual ConsumeCount CHILD_ConsumeData(
        std::vector<CanFdFrame>* replies, int expected_ok_count,
    ...
    )
    {
        ...
        const auto advance = t_options_.max_pipeline < 0 ? size : t_options_.max_pipeline;

        for (size_t start = 0; start < size; start += advance) {
            int expected_ok_count = 0;
            for (auto& v : expected_reply_count_) {
                v = 0;
            }
            ...
            CHILD_CheckReplies(replies, kWait, expected_ok_count, &expected_reply_count_);
        }
    ...
    void CHILD_CheckReplies(
        std::vector<CanFdFrame>* replies, ReadDelay read_delay, int expected_ok_count,
        std::vector<int>* expected_reply_count
    )
    {
        ...
        auto end_time = start + (read_delay == kWait
                                     ? std::max(
                                           expected_ok_count != 0 ? t_options_.min_ok_wait_ns : 0,
        ...
        while (true) {
            ...
            const auto consume_count =
                CHILD_ConsumeData(replies, expected_ok_count, expected_reply_count);

            ok_count += consume_count.ok;

            if (read_delay != kFlush && !any_reply_checker() && ok_count >= expected_ok_count) {
...
public:
    ...
    static std::string DetectFdcanusb()
    {
        ...
        {
            glob_t glob_data = {};
            const int result = ::glob("/dev/serial/by-id/*fdcanusb*", 0, nullptr, &glob_data);
...
private:
    ...
    virtual int CHILD_GetReadFd() const override { return read_fd_; }
    ...
    ) override
    {
        ...
        const int read_ret = ::read(read_fd_, &line_buffer_[line_buffer_pos_], to_read);
        if (read_ret < 0) {
        ...
        const auto consume_count = CHILD_ConsumeLines(replies, expected_reply_count);
    ...
    ConsumeCount
    CHILD_ConsumeLines(std::vector<CanFdFrame>* replies, std::vector<int>* expected_reply_count)
    {
    ...
    bool CHILD_ConsumeLine(
        std::vector<CanFdFrame>* replies, int* ok_count, std::vector<int>* expected_reply_count
    )
    {
        ...
        CHILD_ProcessLine(
            std::string(&line_buffer_[0], line_end), replies, ok_count, expected_reply_count
        );

        std::memmove(
            &line_buffer_[0], &line_buffer_[line_end + 1], line_buffer_pos_ - line_end - 1
        );
    ...
    void CHILD_ProcessLine(
        const std::string& line, std::vector<CanFdFrame>* replies, int* ok_count,
        std::vector<int>* expected_reply_count
    )
    {
        if (line == "OK") {
        ...
        while (true) {
            ...
            if (maybe_flags.empty()) {
                break;
            }
            if (maybe_flags.size() != 1) {
                continue;
            }
            for (const char c : maybe_flags) {
                ...
                if (c == 'B') {
                    this_frame.brs = CanFdFrame::kForceOn;
                }
                if (c == 'f') {
                    this_frame.fdcan_frame = CanFdFrame::kForceOff;
                }
                if (c == 'F') {
                    this_frame.fdcan_frame = CanFdFrame::kForceOn;
                }
            }
        }

        if (expected_reply_count) {
            if (this_frame.source < static_cast<int>(expected_reply_count->size())) {
                (*expected_reply_count)[this_frame.source] =
                    std::max((*expected_reply_count)[this_frame.source] - 1, 0);
            }
        }
...
public:
    ...
    Socketcan(const Options& options): details::TimeoutTransport(options), options_(options)
    {
        socket_ = Open(options_.ifname);
    }

    virtual ~Socketcan() { std::atomic_store(&UNPROTECTED_event_loop_, {}); }
...
private:
    ...
    static int Open(const std::string& ifname)
    {
        ...
        FailIf(::ioctl(fd, SIOCGIFINDEX, &ifr) < 0, "could not find CAN: " + ifname);
    ...
    virtual int CHILD_GetReadFd() const override { return socket_; }
    ...
    virtual void CHILD_SendCanFdFrame(const CanFdFrame& frame) override
    {
        ...
        if (send_frame.len != frame.size) {
            std::memset(&send_frame.data[frame.size], 0x50, send_frame.len - frame.size);
        }

        using F = CanFdFrame;

        send_frame.flags =
            ((frame.fdcan_frame == F::kDefault || frame.fdcan_frame == F::kForceOn) ? CANFD_FDF
                                                                                    : 0) |
    ...
    ) override
    {
        ...
        FailIf(::read(socket_, &recv_frame, sizeof(recv_frame)) < 0, "error reading CAN frame");
        ...
        this_frame.brs =
            (recv_frame.flags & CANFD_BRS) ? CanFdFrame::kForceOn : CanFdFrame::kForceOff;
        this_frame.fdcan_frame =
            (recv_frame.flags & CANFD_FDF) ? CanFdFrame::kForceOn : CanFdFrame::kForceOff;
...
public:
    ...
    using TransportArgPair = std::pair<std::shared_ptr<Transport>, std::vector<std::string>>;
    ...
    struct Argument {
        ...
        Argument(const std::string& name_in, int nargs_in, const std::string& help_in)
        : name(name_in),
          nargs(nargs_in),
          help(help_in)
        {}
...
public:
    ...
    {
        for (const auto& arg : args) {
            if (arg == "--socketcan-iface") {
                return true;
            }
            if (arg == "--socketcan-ignore-errors") {
                return true;
            }
        }
        return false;
...
public:
    template <typename T> void Register() { items_.push_back(std::make_shared<T>()); }
    ...
    std::vector<TransportFactory::Argument> cmdline_arguments() const
    {
        ...
        result.push_back(
            TransportFactory::Argument(
                "--force-transport", 1, "force the given transport type to be used"
            )
        );
