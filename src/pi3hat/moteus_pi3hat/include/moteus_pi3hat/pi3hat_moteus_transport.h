...
public:
      ...
      thread_(std::bind(&Pi3HatMoteusTransport::CHILD_Run, this))
    {
        ...
        Cycle(nullptr, 0, &replies, nullptr, nullptr, &input_override, cbk.callback());
        cbk.Wait();
    }
    ...
    void Cycle(
        const CanFdFrame* frames, size_t size, std::vector<CanFdFrame>* replies,
        pi3hat::Attitude* attitude, pi3hat::Pi3Hat::Output* pi3hat_output,
        pi3hat::Pi3Hat::Input* input_override, moteus::CompletionCallback completed_callback
    )
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if (active_) {
            throw std::logic_error("Cycle cannot be called until the previous has completed");
        }

        cycle_data_ = CycleData{
            frames, size, replies, attitude, pi3hat_output, input_override, completed_callback,
        };
        active_ = true;
...
private:
        ...
        while (true) {
            {
                ...
                condition_.wait(lock, [&]() {
                    return (done_ || active_ || !event_queue_.empty());
                });
    ...
    void CHILD_Cycle()
    {
        ...
        pi3hat::Pi3Hat::Input input =
            cycle_data_.input_override ? *cycle_data_.input_override : options_.default_input;
        ...
        if (cycle_data_.pi3hat_output) {
            *cycle_data_.pi3hat_output = output;
        }
...
public:
    ...
    make(const std::vector<std::string>& args_in) override
    {
        ...
        return std::make_pair(std::make_shared<Pi3HatMoteusTransport>(options), args);
    }
    ...
    virtual bool is_args_set(const std::vector<std::string>& args) override
    {
        for (const auto& arg : args) {
            if (arg == "--pi3hat-cpu" || arg == "--pi3hat-spi-hz" || arg == "--pi3hat-cfg" ||
...
private:
    static std::vector<std::string> Split(const std::string& message, char delim)
    {
