...
public:
    ...
    Controller(const Options& options = {}): options_(options)
    {
        ...
        pd_query_reply_size_ = power_distributor::Query::Make(
            &pd_query_write, options_.power_distributor_query_format
        );
    }
    ...
    void AsyncQuery(
        Result* result, CompletionCallback callback, const Query::Format* format_override = nullptr
    )
    {
    ...
    CanFdFrame MakePDQuery(const power_distributor::Query::Format* format_override = nullptr)
    {
        ...
        if (format_override) {
            ...
            std::memcpy(&result.data[result.size], &pd_query_frame_.data[0], pd_query_frame_.size);
    ...
    void AsyncStop(
        Result* result, CompletionCallback callback, const Query::Format* query_override = nullptr
    )
    {
    ...
    CanFdFrame MakeZeroVelocity(
        const ZeroVelocityMode::Command& cmd = {},
    ...
    )
    {
        return MakeFrame(
            ...
            (command_override == nullptr ? options_.zero_velocity_format : *command_override),
            query_override
        );
    }

    Optional<Result> SetZeroVelocity(
        const ZeroVelocityMode::Command& cmd = {},
        const ZeroVelocityMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    ...
    void AsyncZeroVelocity(
        const ZeroVelocityMode::Command& cmd, Result* result, CompletionCallback callback,
        const ZeroVelocityMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeZeroVelocity(cmd, command_override, query_override), result, callback
        );
    }
    ...
    CanFdFrame MakePosition(
        const PositionMode::Command& cmd, const PositionMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            PositionMode(), cmd,
            (command_override == nullptr ? options_.position_format : *command_override),
            query_override
        );
    }
    ...
    {
        return ExecuteSingleCommand(MakePosition(cmd, command_override, query_override));
    }

    void AsyncPosition(
        const PositionMode::Command& cmd, Result* result, CompletionCallback callback,
        const PositionMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakePosition(cmd, command_override, query_override), result, callback
        );
    }
    ...
    Optional<Result> SetPositionWaitComplete(
        const PositionMode::Command& cmd, double period_s,
        const PositionMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
    ...
    CanFdFrame MakeVFOC(
        const VFOCMode::Command& cmd, const VFOCMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            VFOCMode(), cmd, command_override == nullptr ? options_.vfoc_format : *command_override,
            query_override
        );
    }

    Optional<Result> SetVFOC(
        const VFOCMode::Command& cmd, const VFOCMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
    ...
    void AsyncVFOC(
        const VFOCMode::Command& cmd, Result* result, CompletionCallback callback,
        const VFOCMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(MakeVFOC(cmd, command_override, query_override), result, callback);
    }
    ...
    CanFdFrame MakeCurrent(
        const CurrentMode::Command& cmd, const CurrentMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            CurrentMode(), cmd,
            (command_override == nullptr ? options_.current_format : *command_override),
            query_override
        );
    }
    ...
    void AsyncCurrent(
        const CurrentMode::Command& cmd, Result* result, CompletionCallback callback,
        const CurrentMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeCurrent(cmd, command_override, query_override), result, callback
        );
    }
    ...
    )
    {
        return MakeFrame(
            StayWithinMode(), cmd,
            (command_override == nullptr ? options_.stay_within_format : *command_override),
            query_override
        );
    }
    ...
    void AsyncStayWithin(
        const StayWithinMode::Command& cmd, Result* result, CompletionCallback callback,
        const StayWithinMode::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
    ...
    CanFdFrame MakeOutputNearest(
        const OutputNearest::Command& cmd, const OutputNearest::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            OutputNearest(), cmd,
            (command_override == nullptr ? OutputNearest::Format() : *command_override),
            query_override
        );
    }

    Optional<Result> SetOutputNearest(
        const OutputNearest::Command& cmd, const OutputNearest::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
    ...
    void AsyncOutputNearest(
        const OutputNearest::Command& cmd, Result* result, CompletionCallback callback,
        const OutputNearest::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeOutputNearest(cmd, command_override, query_override), result, callback
        );
    }
    ...
    CanFdFrame MakeOutputExact(
        const OutputExact::Command& cmd, const OutputExact::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            OutputExact(), cmd,
            (command_override == nullptr ? OutputExact::Format() : *command_override),
            query_override
        );
    }

    Optional<Result> SetOutputExact(
        const OutputExact::Command& cmd, const OutputExact::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
    ...
    void AsyncOutputExact(
        const OutputExact::Command& cmd, Result* result, CompletionCallback callback,
        const OutputExact::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeOutputExact(cmd, command_override, query_override), result, callback
        );
    }
    ...
    CanFdFrame MakeRequireReindex(
        const RequireReindex::Command& cmd = {},
    ...
    )
    {
        return MakeFrame(
            RequireReindex(), cmd,
            (command_override == nullptr ? RequireReindex::Format() : *command_override),
            query_override
        );
    }

    Optional<Result> SetRequireReindex(
        const RequireReindex::Command& cmd = {},
        const RequireReindex::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    ...
    void AsyncRequireReindex(
        const RequireReindex::Command& cmd, Result* result, CompletionCallback callback,
        const RequireReindex::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeRequireReindex(cmd, command_override, query_override), result, callback
        );
    }
    ...
    {
        return MakeFrame(
            RecapturePositionVelocity(), cmd,
            (command_override == nullptr ? RecapturePositionVelocity::Format() : *command_override),
            query_override
        );
    }
    ...
    void AsyncRecapturePositionVelocity(
        const RecapturePositionVelocity::Command& cmd, Result* result, CompletionCallback callback,
        const RecapturePositionVelocity::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeRecapturePositionVelocity(cmd, command_override, query_override), result, callback
        );
    }
    ...
    CanFdFrame MakeClockTrim(
        const ClockTrim::Command& cmd, const ClockTrim::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            ClockTrim(), cmd,
            (command_override == nullptr ? ClockTrim::Format() : *command_override), query_override
        );
    }

    Optional<Result> SetClockTrim(
        const ClockTrim::Command& cmd, const ClockTrim::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return ExecuteSingleCommand(MakeClockTrim(cmd, command_override, query_override));
    }

    void AsyncClockTrim(
        const ClockTrim::Command& cmd, Result* result, CompletionCallback callback,
        const ClockTrim::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeClockTrim(cmd, command_override, query_override), result, callback
        );
    }
    ...
    std::string
    DiagnosticCommand(const std::string& message, DiagnosticReplyMode reply_mode = kExpectOK)
    {
    ...
    void AsyncDiagnosticCommand(
        const std::string& message, std::string* result, CompletionCallback callback,
    ...
    void AsyncDiagnosticWrite(const std::string& message, int channel, CompletionCallback callback)
    {
    ...
    void AsyncDiagnosticRead(std::string* response, int channel, CompletionCallback callback)
    {
        ...
        context->transport->Cycle(&output_frame_, 1, &context->replies, [context, this](int) {
            std::string response;
            bool any_response = false;

            for (const auto& frame : context->replies) {
                if (frame.destination != options_.source || frame.source != options_.id ||
                    frame.can_prefix != options_.can_prefix) {
                    continue;
                }
                auto maybe_data = DiagnosticResponse::Parse(frame.data, frame.size);
                response +=
                    std::string(reinterpret_cast<const char*>(maybe_data.data), maybe_data.size);
                any_response = true;
            }
    ...
    CanFdFrame MakeWriteGpio(
        const GpioWrite::Command& cmd, const GpioWrite::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            GpioWrite(), cmd,
            (command_override == nullptr ? GpioWrite::Format() : *command_override), query_override
        );
    }

    Optional<Result> SetWriteGpio(
        const GpioWrite::Command& cmd, const GpioWrite::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return ExecuteSingleCommand(MakeWriteGpio(cmd, command_override, query_override));
    }

    void AsyncWriteGpio(
        const GpioWrite::Command& cmd, Result* result, CompletionCallback callback,
        const GpioWrite::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeWriteGpio(cmd, command_override, query_override), result, callback
        );
    }
    ...
    CanFdFrame MakeGpioRead(
        const GpioRead::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            GpioRead(), GpioRead::Command(),
            (command_override == nullptr ? GpioRead::Format() : *command_override), query_override
        );
    }

    Optional<Result> SeGpioRead(
        const GpioRead::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return ExecuteSingleCommand(MakeGpioRead(command_override, query_override));
    }
    ...
    {
        AsyncStartSingleCommand(MakeGpioRead(command_override, query_override), result, callback);
    }
    ...
    CanFdFrame MakeAuxPwmWrite(
        const AuxPwmWrite::Command& cmd, const AuxPwmWrite::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        return MakeFrame(
            AuxPwmWrite(), cmd,
            (command_override == nullptr ? AuxPwmWrite::Format() : *command_override),
            query_override
        );
    }

    Optional<Result> SetAuxPwmWrite(
        const AuxPwmWrite::Command& cmd, const AuxPwmWrite::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
    ...
    void AsyncAuxPwmWrite(
        const AuxPwmWrite::Command& cmd, Result* result, CompletionCallback callback,
        const AuxPwmWrite::Format* command_override = nullptr,
        const Query::Format* query_override = nullptr
    )
    {
        AsyncStartSingleCommand(
            MakeAuxPwmWrite(cmd, command_override, query_override), result, callback
        );
    }
    ...
    void AsyncVerifySchemaVersion(CompletionCallback callback)
    {
        ...
        AsyncStartSingleCommand(
            MakeSchemaVersionQuery(), result.get(), [result, callback, t](int value) {
                CheckRegisterMapVersion(*result);
    ...
    void AsyncStartSingleCommand(const CanFdFrame& cmd, Result* result, CompletionCallback callback)
    {
        auto t = transport();
        output_frame_ = cmd;
        t->Cycle(&output_frame_, 1, &single_command_replies_, [callback, result, this, t](int) {
    ...
    static void DefaultArgProcess(const std::vector<std::string>& args)
    {
        if (std::find(args.begin() + 1, args.end(), "--help") != args.end() ||
            ...
            auto help_strs = moteus::TransportRegistry::singleton().cmdline_arguments();
            help_strs.insert(
                help_strs.begin(),
                TransportFactory::Argument("--help", 0, "Display this usage message")
            );
    ...
    static std::vector<std::string> ProcessTransportArgs(const std::vector<std::string>& args)
    {
...
private:
    ...
    ArgumentProcessHelper(const std::vector<std::string>& args)
    {
        auto& g_transport = *GlobalTransport();
        if (g_transport) {
            return std::make_pair(g_transport, args);
        }
    ...
    : public std::enable_shared_from_this<AsyncDiagnosticCommandContext> {
        ...
        void DoWrite()
        {
            ...
            controller->transport()->Cycle(&output_frame_, 1, nullptr, [s, to_write](int v) {
        ...
        void DoRead()
        {
            ...
            transport->Cycle(&output_frame_, 1, &replies, [s](int v) { s->Callback(v); });
        }
        ...
        bool ProcessReplies()
        {
            for (const auto& reply : replies) {
                ...
                current_line +=
                    std::string(reinterpret_cast<const char*>(parsed.data), parsed.size);
            }
    ...
    {
        if (result.values.extra[0].register_number != Register::kRegisterMapVersion) {
        ...
        if (kCurrentRegisterMapVersion != int_version) {
            ...
            ostr << "Register map version mismatch device is " << int_version
                 << " but library requires " << kCurrentRegisterMapVersion;
    ...
    Optional<Result> FindResult(const std::vector<CanFdFrame>& replies) const
    {
        ...
        for (auto it = replies.rbegin(); it != replies.rend(); ++it) {
            if (it->source == options_.id && it->destination == options_.source &&
    ...
    CanFdFrame MakeFrame(
        const CommandType&, const typename CommandType::Command& cmd,
    ...
    )
    {
        ...
        if (query_format_override) {
            result.expected_reply_size = Query::Make(&write_frame, *query_format_override);
        } else if (options_.default_query) {
            std::memcpy(&result.data[result.size], &query_frame_.data[0], query_frame_.size);
