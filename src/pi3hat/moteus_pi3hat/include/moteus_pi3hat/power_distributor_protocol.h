    ...
    static uint8_t Make(moteus::WriteCanData* frame, const Format& format)
    {
        uint8_t reply_size = 0;
        {
            const moteus::Resolution kResolutions[] = {
                format.state, format.fault, format.switch_status, format.lock_time, format.boot_time
            };
            const uint16_t kResolutionSize = sizeof(kResolutions) / sizeof(*kResolutions);
            moteus::WriteCombiner combiner(
                frame, 0x10, DRegister::kState, kResolutions, kResolutionSize
            );
        ...
        {
            const moteus::Resolution kResolutions[] = {
                format.output_voltage, format.output_current, format.temperature, format.energy
            };
            const uint16_t kResolutionSize = sizeof(kResolutions) / sizeof(*kResolutions);
            moteus::WriteCombiner combiner(
                frame, 0x10, DRegister::kOutputVoltage, kResolutions, kResolutionSize
            );
...
struct StateCommand {
    ...
    static uint8_t Make(moteus::WriteCanData* frame, const Command& command, const Format& format)
    {
        frame->Write<int8_t>(moteus::Multiplex::kWriteInt8 | 0x01);
        frame->Write<int8_t>(power_distributor::DRegister::kState);
        frame->Write<int8_t>(
            command.state == 0 ? power_distributor::State::kPowerOff
                               : power_distributor::State::kPowerOn
        );
        return 0;
    };
};
