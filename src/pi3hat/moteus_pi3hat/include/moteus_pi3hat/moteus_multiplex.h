...
#error "only little endian architectures supported"
        ...
        ::memcpy(&data_[*size_], reinterpret_cast<const char*>(&value), sizeof(value));
        *size_ += sizeof(value);
    }
    ...
    void WriteInt(int32_t value, Resolution res)
    {
        ...
        case Resolution::kInt8: {
            Write<int8_t>(detail::max<int32_t>(-127, detail::min<int32_t>(127, value)));
            break;
        }
        case Resolution::kInt16: {
            Write<int16_t>(detail::max<int32_t>(-32767, detail::min<int32_t>(32767, value)));
            break;
        }
    ...
    void WriteMapped(
        double value, double int8_scale, double int16_scale, double int32_scale, Resolution res
    )
    {
        switch (res) {
        case Resolution::kInt8: {
    ...
    void WriteAccel(double value, Resolution res) { WriteMapped(value, 0.05, 0.001, 0.00001, res); }

    void WriteTorque(double value, Resolution res) { WriteMapped(value, 0.5, 0.01, 0.001, res); }
    ...
    void WriteVoltage(double value, Resolution res) { WriteMapped(value, 0.5, 0.1, 0.001, res); }

    void WriteTemperature(float value, Resolution res) { WriteMapped(value, 1.0, 0.1, 0.001, res); }

    void WriteTime(double value, Resolution res) { WriteMapped(value, 0.01, 0.001, 0.000001, res); }

    void WriteCurrent(double value, Resolution res) { WriteMapped(value, 1.0, 0.1, 0.001, res); }
...
public:
    MultiplexParser(const CanData* frame): data_(&frame->data[0]), size_(frame->size) {}
    MultiplexParser(const uint8_t* data, uint8_t size): data_(data), size_(size) {}
    ...
    int ReadInt(Resolution res) { return static_cast<int>(ReadConcrete(res, kInt)); }
    ...
    double ReadTemperature(Resolution res) { return ReadConcrete(res, kTemperature); }
    ...
    double ReadEnergy(Resolution res) { return ReadConcrete(res, kEnergy); }
...
public:
    WriteCombiner(
        WriteCanData* frame, int8_t base_command, uint16_t start_register,
        const Resolution* resolutions, uint16_t resolutions_size
    )
    : frame_(frame),
