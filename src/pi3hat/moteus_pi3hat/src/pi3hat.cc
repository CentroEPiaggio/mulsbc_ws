...
void copy_data(void* ptr) { ::memcpy(g_data_block, ptr, sizeof(g_data_block)); }
...
template <typename ErrorGenerator> void ThrowIf(bool value, ErrorGenerator error_generator)
{
    if (!value) {
        return;
    }
...
public:
    ...
    PrimarySpi(const Options& options = Options())
    {
        ...
        spi_mmap_ = SystemMmap(fd_, 4096, host_get_peripheral_address() + SPI_BASE);
        spi_ = reinterpret_cast<volatile Bcm2835Spi*>(static_cast<char*>(spi_mmap_.ptr()));
    ...
    Rpi3Gpio* gpio() { return gpio_.get(); }
    ...
    void Read(int cs, int address, char* data, size_t size)
    {
        ...
        if (size != 0) {
            ...
            while (remaining_read) {
                ...
                if (can_write && remaining_write && (spi_->cs & SPI_CS_TXD) != 0) {
                    spi_->fifo = 0x00;
                    remaining_write--;
                }
...
public:
    ...
    AuxSpi(const Options& options = Options())
    {
        ...
        spi_mmap_ = SystemMmap(fd_, 4096, host_get_peripheral_address() + AUX_BASE);
        auxenb_ = reinterpret_cast<volatile uint32_t*>(static_cast<char*>(spi_mmap_.ptr()) + 0x04);
        spi_ =
            reinterpret_cast<volatile Bcm2835AuxSpi*>(static_cast<char*>(spi_mmap_.ptr()) + 0x80);
        ...
        *auxenb_ = (*auxenb_ | 0x02); // SPI1 enable
        ...
        // Configure the SPI peripheral.
        const int clkdiv = std::max(0, std::min(4095, 250000000 / 2 / options.speed_hz - 1));
        spi_->cntl0 = (
        0
        | (clkdiv << 20)
...
struct DeviceImuConfiguration {
    ...
    bool operator!=(const DeviceImuConfiguration& rhs) const { return !(*this == rhs); }
} __attribute__((packed));
...
struct DeviceCanRate {
    ...
    {
        return prescaler == rhs.prescaler && sync_jump_width == rhs.sync_jump_width &&
               time_seg1 == rhs.time_seg1 && time_seg2 == rhs.time_seg2;
    }

    bool operator!=(const DeviceCanRate& rhs) const { return !(*this == rhs); }
} __attribute__((packed));
...
struct DeviceCanConfiguration {
    ...
    {
        return slow_bitrate == rhs.slow_bitrate && fast_bitrate == rhs.fast_bitrate &&
               fdcan_frame == rhs.fdcan_frame && bitrate_switch == rhs.bitrate_switch &&
               automatic_retransmission == rhs.automatic_retransmission &&
               restricted_mode == rhs.restricted_mode && bus_monitor == rhs.bus_monitor &&
               std_rate == rhs.std_rate && fd_rate == rhs.fd_rate;
    }

    bool operator!=(const DeviceCanConfiguration& rhs) const { return !(*this == rhs); }
} __attribute__((packed));
...
struct DeviceCanConfigurationV4: DeviceCanConfiguration {
    ...
    {
        return DeviceCanConfiguration::operator==(rhs) && cancel_all_ms == rhs.cancel_all_ms;
    }

    bool operator!=(const DeviceCanConfigurationV4& rhs) const { return !(*this == rhs); }
} __attribute__((packed));

template <typename Spi> Pi3Hat::ProcessorInfo GetProcessorInfo(Spi* spi, int cs)
{
    DeviceDeviceInfo di;
    ...
    ::memcpy(&result.serial_number[0], &di.serial_number[0], sizeof(di.serial_number));
    return result;
}

template <typename Spi> Pi3Hat::PerformanceInfo GetPerformance(Spi* spi, int cs)
{
...
public:
      ...
      }()}
    {
        ...
        const auto product_code = ReadContents("/sys/firmware/devicetree/base/hat/product");
    ...
    void ConfigureAux()
    {
        ...
        primary_spi_.Read(
            0, 35, reinterpret_cast<char*>(&original_imu_configuration),
        ...
        desired_imu.rate_hz = std::min<uint32_t>(1000, config_.attitude_rate_hz);

        if (desired_imu != original_imu_configuration) {
            primary_spi_.Write(
                0, 36, reinterpret_cast<const char*>(&desired_imu), sizeof(desired_imu)
            );
            ...
            primary_spi_.Read(
                0, 35, reinterpret_cast<char*>(&config_verify), sizeof(config_verify)
            );
            ThrowIf(desired_imu != config_verify, [&]() {
                return Format(
                    "IMU config not set properly (%f,%f,%f) %d != (%f,%f,%f) %d",
                    desired_imu.yaw_deg, desired_imu.pitch_deg, desired_imu.roll_deg,
                    desired_imu.rate_hz, config_verify.yaw_deg, config_verify.pitch_deg,
                    config_verify.roll_deg, config_verify.rate_hz
                );
            });
        }
        ...
        primary_spi_.Read(0, 49, reinterpret_cast<char*>(&original_id), sizeof(original_id));
        if (original_id != config_.rf_id) {
            primary_spi_.Write(
                0, 50, reinterpret_cast<const char*>(&config_.rf_id), sizeof(uint32_t)
            );
            // Changing the ID takes at least a few milliseconds.
            ::usleep(10000);
            uint32_t id_verify = 0;
            primary_spi_.Read(0, 49, reinterpret_cast<char*>(&id_verify), sizeof(id_verify));
            ThrowIf(config_.rf_id != id_verify, [&]() {
                return Format("RF Id not set properly (%08x != %08x)", config_.rf_id, id_verify);
            });
        }
    }

    template <typename Spi>
    void UpdateCanConfig(Spi* spi, int cs, int canbus, const CanConfiguration& can_config)
    {
        ...
        const auto spi_size =
            version <= 3 ? sizeof(DeviceCanConfiguration) : sizeof(DeviceCanConfigurationV4);
        spi->Read(cs, canbus ? 8 : 7, reinterpret_cast<char*>(&original_config), spi_size);
        ...
        spi->Write(cs, canbus ? 10 : 9, reinterpret_cast<const char*>(&out), spi_size);
        ...
        if (version <= 3) {
            ...
            spi->Read(cs, canbus ? 8 : 7, reinterpret_cast<char*>(&verify), spi_size);
            ThrowIf(static_cast<DeviceCanConfiguration>(out) != verify, [&]() {
                return "Could not set CAN configuration properly";
            });
        } else {
            DeviceCanConfigurationV4 verify;
            spi->Read(cs, canbus ? 8 : 7, reinterpret_cast<char*>(&verify), spi_size);
            ThrowIf(out != verify, [&]() { return "Could not set CAN configuration properly"; });
        }
    }
    ...
    void LockFile()
    {
        ...
        lock_file_fd_ =
            ::open("/tmp/.pi3hat-lock", O_RDWR | O_CREAT, S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP);
    ...
    template <typename Spi> void TestCan(Spi* spi, int cs, const char* name)
    {
        ...
        if (version != 2 && version != 3 && version != 4) {
            throw std::runtime_error(
                Format("Processor '%s' has incorrect CAN SPI version %d != [2,3,4]", name, version)
            );
        }
    }
        ...
        if (config_.enable_aux) {
            ...
            if (attitude_version != kAttitudeVersion) {
                throw std::runtime_error(Format(
                    "Incorrect attitude version %d != %d", attitude_version, kAttitudeVersion
                ));
            }
            ...
            if (rf_version != kRfVersion) {
                throw std::runtime_error(
                    Format("Incorrect RF version %d != %d", rf_version, kRfVersion)
                );
            }
        }
    }
    ...
    DeviceInfo device_info()
    {
        ...
        const auto aux_can_protocol =
            config_.enable_aux ? ReadByte(&primary_spi_, 0, 0) : can1_can_protocol;

        result.can_unknown_address_safe =
            (can1_can_protocol > 3) && (can2_can_protocol > 3) && (aux_can_protocol > 3);
    ...
    bool GetAttitude(Attitude* output, bool wait, bool detail)
    {
        ...
        do {
            primary_spi_.Read(
                0, 34, reinterpret_cast<char*>(&device_attitude_),
    ...
    template <typename Spi>
    void SendCanPacketSpi(Spi& spi, int cs, int cpu_bus, const CanFrame& can_frame)
    {
    ...
    ExpectedReply CalculateExpectedReply(const Input& input)
    {
        ...
        for (size_t i = 0; i < input.tx_can.size(); i++) {
            ...
            const int64_t arbitration_bitrate = config_.can[bus - 1].slow_bitrate;
            const int64_t data_bitrate = config_.can[bus - 1].bitrate_switch
                                             ? config_.can[bus - 1].fast_bitrate
                                             : config_.can[bus - 1].slow_bitrate;

            const int64_t can_header_size_bits = (input.tx_can[i].id >= 2048 ? 32 : 16) + 8;
            const int64_t can_data_size_bits = input.tx_can[i].size * 8 + 28;
            const int64_t tx_spi_bits = (input.tx_can[i].size + 5) * 8;
            const int64_t can_send_ns = 1000000000 * can_header_size_bits / arbitration_bitrate +
            ...
            if (input.tx_can[i].expect_reply) {
                ...
                const int64_t rx_data_size_bits = input.tx_can[i].expected_reply_size * 8 + 28;
                const int64_t rx_spi_bits = (input.tx_can[i].expected_reply_size + 5) * 8;
                const int64_t rx_ns = 1000000000 * rx_header_size_bits / arbitration_bitrate +
    ...
    void SendRf(const Span<RfSlot>& slots)
    {
        ...
        for (size_t i = 0; i < slots.size(); i++) {
            ...
            primary_spi_.Write(
                0, 51, reinterpret_cast<const char*>(&buf[0]), kHeaderSize + slot.size
            );
        }
    }
    ...
    void ReadRf(const Input& input, Output* output)
    {
        ...
        primary_spi_.Read(0, 52, reinterpret_cast<char*>(&rf_status), sizeof(rf_status));
        ...
        for (int i = 0; i < 15; i++) {
            ...
            primary_spi_.Read(0, 64 + i, reinterpret_cast<char*>(&slot_data), sizeof(slot_data));
    ...
    template <typename Spi>
    int ReadCanFrames(Spi& spi, int cs, int bus_start, const Span<CanFrame>* rx_can, Output* output)
    {
        // Is there any room?
        ...
        while (true) {
            ...
            spi.Read(cs, 2, reinterpret_cast<char*>(&queue_sizes[0]), sizeof(queue_sizes));
            ...
            for (int size : queue_sizes) {
                ...
                output_frame.id = (buf[1] << 24) | (buf[2] << 16) | (buf[3] << 8) | (buf[4] << 0);
    ...
    void FlushReadCan(const Input& input, const ExpectedReply& expected_replies, Output* output)
    {
        const auto can1_expected =
            (expected_replies.count[1] + expected_replies.count[2]) || input.force_can_check & 0x06;
        ...
        const auto can2_expected =
            (expected_replies.count[3] + expected_replies.count[4]) || input.force_can_check & 0x18;
        if (can2_expected) {
            ReadCanFrames(aux_spi_, 1, 3, &input.rx_can, output);
        }

        const auto aux_expected =
            ((expected_replies.count[5]) || input.force_can_check & 0x20) && config_.enable_aux;
        if (aux_expected) {
            ReadCanFrames(primary_spi_, 0, 5, &input.rx_can, output);
        }
    }

    void ReadCan(const Input& input, const ExpectedReply& expected_replies, Output* output)
    {
        int bus_replies[] = {
        ...
        while (true) {
            ...
            if (bus_replies[0] <= 0 && bus_replies[1] <= 0 && bus_replies[2] <= 0 &&
                delta_ns > input.min_tx_wait_ns && since_last_ns > input.rx_extra_wait_ns) {
            ...
            if (delta_ns > input.timeout_ns &&
                !(delta_ns < input.min_tx_wait_ns || delta_ns < min_rx_timeout_ns ||
    ...
    Output Cycle(const Input& input)
    {
        ...
        if (input.request_attitude) {
            result.attitude_present =
                GetAttitude(input.attitude, input.wait_for_attitude, input.request_attitude_detail);
        }
...
Pi3Hat::Pi3Hat(const Configuration& configuration): impl_(new Impl(configuration)) {}

Pi3Hat::~Pi3Hat() { delete impl_; }

Pi3Hat::Output Pi3Hat::Cycle(const Input& input) { return impl_->Cycle(input); }

Pi3Hat::DeviceInfo Pi3Hat::device_info() { return impl_->device_info(); }

Pi3Hat::DevicePerformance Pi3Hat::device_performance() { return impl_->device_performance(); }
