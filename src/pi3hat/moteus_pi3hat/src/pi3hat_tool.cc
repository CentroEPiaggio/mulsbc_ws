...
///
/// g++ -mcpu=cortex-a53 -L /opt/vc/lib -I /opt/vc/include -std=c++11 pi3hat_tool.cc pi3hat.cc -o
/// pi3hat_tool
...
public:
    ...
    Quaternion(): w_(1.0), x_(0.), y_(0.), z_(0.) {}

    std::string str() const;

    Quaternion conjugated() const { return Quaternion(w_, -x_, -y_, -z_); }
    ...
    static Quaternion FromEuler(double roll_rad, double pitch_rad, double yaw_rad)
    {
        // Quaternions multiply in opposite order, and we want to get into
        // roll, pitch, then yaw as standard.
        return (
            Quaternion::FromAxisAngle(yaw_rad, 0, 0, 1) *
    ...
    static Quaternion FromAxisAngle(double angle_rad, double x, double y, double z)
    {
...
inline Quaternion operator*(const Quaternion& lhs, const Quaternion& rhs)
{
    ...
    return Quaternion(
        a * e - b * f - c * g - d * h, b * e + a * f + c * h - d * g, a * g - b * h + c * e + d * f,
        a * h + b * g - c * f + d * e
    );
}
...
template <typename Iterator> Pi3Hat::CanConfiguration ParseCanConfig(Iterator begin, Iterator end)
{
    ...
    for (auto it = begin; it != end; ++it) {
        ...
        } else if (item.at(0) == 's' || item.at(0) == 'f') {
...
Pi3Hat::Configuration MakeConfig(const Arguments& args)
{
    ...
    for (const auto& can_config : args.can_config) {
        ...
        config.can[can_bus - 1] = ParseCanConfig(can_fields.begin() + 1, can_fields.end());
    }
...
void CheckError(int error)
{
    ...
    throw std::runtime_error("Unexpected pi3hat error: " + std::to_string(error));
}
...
{
    if ((size % 2) != 0) {
        throw std::runtime_error("CAN data invalid length: " + std::string(data, size));
    }
    size_t bytes = 0;
...
public:
    MiniTokenizer(const std::string& data): data_(data) { find_next(); }
...
CanFrame ParseCanString(const std::string& can_string)
{
    ...
    tok.advance();
    result.size = ParseHexData(result.data, &can_string[tok.pos], tok.next - tok.pos);
...
RfSlot ParseRfString(const std::string& rf_string)
{
    ...
    tok.advance();
    result.size = ParseHexData(result.data, &rf_string[tok.pos], tok.next - tok.pos);
...
double R2D(double value) { return 180.0 * value / 3.141592653589793; }

std::string FormatAttitude(const Attitude& a)
{
    const auto euler_rad =
        Quaternion(a.attitude.w, a.attitude.x, a.attitude.y, a.attitude.z).euler_rad();
    ...
    ::snprintf(
        ...
        "a=(%4.1f,%4.1f,%4.1f)",
        R2D(euler_rad.roll), R2D(euler_rad.pitch), R2D(euler_rad.yaw), a.rate_dps.x, a.rate_dps.y,
        a.rate_dps.z, a.accel_mps2.x, a.accel_mps2.y, a.accel_mps2.z
    );
...
void Run(Pi3Hat* pi3hat, const Arguments& args)
{
    ...
    while (true) {
        ...
        {
            ::snprintf(buf, sizeof(buf) - 1, "%5.1f Hz  \r", 1.0 / filtered_period_s);
            std::cout << buf;
            std::cout.flush();
        }
...
void ReadSpi(Pi3Hat* pi3hat, const std::string& command)
{
    ...
    std::cout << FormatHexBytes(reinterpret_cast<uint8_t*>(&data[0]), data.size()) << "\n";
}

std::string FormatPerformance(const Pi3Hat::PerformanceInfo& p)
{
    return "average:" + std::to_string(p.cycles_per_ms) + " " +
...
int main(int argc, char** argv) { return mjbots::pi3hat::do_main(argc, argv); }
